...
def _text_ink_rect(label: QtWidgets.QLabel) -> QtCore.QRectF:
    return QtGui.QFontMetricsF(label.font()).tightBoundingRect(label.text())
...
def _title_text_fits(label: QtWidgets.QLabel) -> bool:
    return _text_ink_rect(label).width() <= label.width() + 1.0


def _legend_label_limits(entry) -> tuple[float, float]:
    rect = entry.label.geometry()
    if entry._vertical_label:
        return (float(rect.height()), float(rect.width()))
...
def test_ptable_headers_and_legend_fonts_fit_on_font_change(
    qtbot,
    monkeypatch,
) -> None:
    monkeypatch.setattr(erlab.analysis.xps, "get_edge", lambda _symbol: {"1s": 10.0})
    ...
    qtbot.waitUntil(lambda: win.inspector.plot_target_combo.isVisible())

    new_font = _stress_font(win.font())
    win.inspector.setFont(new_font)
    win.category_legend.setFont(new_font)
    QtWidgets.QApplication.processEvents()
    win.inspector._update_section_header_fonts()
    win.category_legend._update_entry_label_fonts()
    QtWidgets.QApplication.processEvents()

    qtbot.waitUntil(
        lambda: (
            _title_text_fits(win.inspector.plot_title)
            and _title_text_fits(win.inspector.levels_title)
        )
    )
    qtbot.waitUntil(
        lambda: (
            ...
            and win.inspector.levels_title.geometry().right()
            < win.inspector.levels_controls_frame.geometry().left()
        )
    )

    assert round(win.inspector.plot_title.font().pointSizeF(), 2) == round(
        win.inspector.levels_title.font().pointSizeF(), 2
    )

    legend_point_sizes = {
        round(entry.label.font().pointSizeF(), 2)
        for entry in win.category_legend.entries.values()
    }
    assert len(legend_point_sizes) == 1

    alkaline_entry = win.category_legend.entries["alkaline_earth_metal"]
    legend_label_width, legend_label_height = _legend_label_limits(alkaline_entry)
    alkaline_ink_rect = _text_ink_rect(alkaline_entry.label)
    assert alkaline_ink_rect.width() <= legend_label_width + 1.0
    assert alkaline_ink_rect.height() <= legend_label_height + 1.0
    assert alkaline_entry.label.geometry().left() > 0
