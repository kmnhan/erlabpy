...
- Specialized helpers such as {ref}`ktool <guide-ktool>` (momentum conversion), {ref}`dtool <guide-dtool>` (derivative plotting), and the {ref}`data explorer <guide-restool>` integrate with both ImageTool and the manager for specific analysis tasks. These are described in {ref}`interactive-misc-tools <guide-interactive-misc-tools>`. Each tool can be launched directly or from ImageTool's context menus, and the manager can keep them alongside ImageTool windows when you need a shared workspace.
